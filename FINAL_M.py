...
def PRE4(date: str, rol: str, address: str, rut: str, full_name: str, nationality: str, birth_date: str, profession: str, salary: str) -> None:
    document = Document()
    ###########################################
    ########   DOCUMENT PARAMETERS    #########
    ###########################################
...
def PRE4A():
    while True:
        ...
        if not df.empty:
            ...
            PRE4(date, rol, address, rut, full_name, nationality, birth_date, profession, salary)
...
def generate_contracts_for_range(start_rut: str, end_rut: str) -> None:
    conn = sql.connect('DATA/db_personas.db')
    
    query = """
    SELECT p.fecha_ingreso, s.Rol, p.residencia, p.rut, p.nombre_completo, p.nacionalidad, p.fecha_de_nacimiento, p.profesion, s.Sueldo
    FROM Salarios AS s
    INNER JOIN Personas AS p ON p.id_rol = s.id_salarios
    WHERE p.rut BETWEEN ? AND ?
    """

    df = pd.read_sql_query(query, conn, params=(start_rut, end_rut))
    ...
    if not df.empty:
        ...
        print(f"No se encontraron datos para el rango de RUT {start_rut} a {end_rut}.")


def a5():
    while True:
        start_rut = input("Ingrese el RUT de inicio (o 'salir' para terminar): ")
        if start_rut.lower() == 'salir':
            break
        else:
            end_rut = input("Ingrese el RUT de final: ")
            generate_contracts_for_range(start_rut, end_rut)
            break
